...
class PackableTests: XCTestCase {
    ...
    func testPackingArrays() {
        let ints = [-1, 2, 3, 4, -5]
        XCTAssertEqual(radix(ints.pack()), "95 ff 02 03 04 fb")
        let doubles = [1.0132343436, 0.125, 2.1, -3.14]
        XCTAssertEqual(radix(doubles.pack()), "94 cb 3f f0 36 35 37 0f 22 ed cb 3f c0 00 00 00 00 00 00 cb 40 00 cc cc cc cc cc cd cb c0 09 1e b8 51 eb 85 1f")
        let floats = [1.125, 0.25, -0.5].map { Float($0) }
        XCTAssertEqual(radix(floats.pack()), "93 ca 3f 90 00 00 ca 3e 80 00 00 ca bf 00 00 00")
        let strs = ["😡😂", "a", "中", "ABcd"]
        XCTAssertEqual(radix(strs.pack()), "94 a8 f0 9f 98 a1 f0 9f 98 82 a1 61 a3 e4 b8 ad a4 41 42 63 64")
        let uints = [0xab, 0xf1, 0xab_cd, 0x7f_ff_12_34].map { UInt($0) }
        XCTAssertEqual(radix(uints.pack()), "94 cc ab cc f1 cd ab cd ce 7f ff 12 34")
    ...
    func testPackingBinaries() {
        ...
        XCTAssertTrue(radix(bin8.pack()).hasPrefix(radix([FormatMark.bin8.rawValue])))
        XCTAssertEqual(bin8.pack().count, count + 2)
        count = 0x1012
        let bin16 = Binary(bytes: Bytes(repeating: UInt8(arc4random_uniform(0xff)), count: count))
        XCTAssertTrue(radix(bin16.pack()).hasPrefix(radix([FormatMark.bin16.rawValue])))
        XCTAssertEqual(bin16.pack().count, count + 3)
        count = 0x10001
        let bin32 = Binary(bytes: Bytes(repeating: UInt8(arc4random_uniform(0xff)), count: count))
        XCTAssertTrue(radix(bin32.pack()).hasPrefix(radix([FormatMark.bin32.rawValue])))
        XCTAssertEqual(bin32.pack().count, count + 5)
    }

    func testPackingBools() {
        var b = true
        XCTAssertEqual(radix(b.pack()), "c3")
        b = false
        XCTAssertEqual(radix(b.pack()), "c2")
    }

    func testPackingDictionary() {
        let dic: [MPValue: MPValue] = ["😇": [1: true]]
        XCTAssertEqual(radix(dic.pack()), "81 a4 f0 9f 98 87 81 01 c3")
        let strInt = ["a": 1]
        XCTAssertEqual(radix(strInt.pack()), "81 a1 61 01")
        let intStr = [1: "a"]
        XCTAssertEqual(radix(intStr.pack()), "81 01 a1 61")
    }

    func testPackingDoubles() {
        let a = 3.1415926535
        XCTAssertEqual(radix(a.pack()), "cb 40 09 21 fb 54 41 17 44")
    }
    ...
    func testPackingFloats() {
        let a: Float = 3.14
        XCTAssertEqual(radix(a.pack()), "ca 40 48 f5 c3")
    }

    func testPackingInts() {
        var int = -3
        XCTAssertEqual(radix(int.pack()), "fd")
        int = -125
        XCTAssertEqual(radix(int.pack()), "d0 83")
        int = -127
        XCTAssertEqual(radix(int.pack()), "d0 81")
        int = -375
        XCTAssertEqual(radix(int.pack()), "d1 fe 89")
        int = -45687
        XCTAssertEqual(radix(int.pack()), "d2 ff ff 4d 89")
        let int64 = Int64.min + Int64(100)
        XCTAssertEqual(radix(int64.pack()), "d3 80 00 00 00 00 00 00 64")
    }
    ...
    func testPackingMPValues() {
        ...
        XCTAssertEqual(radix(mpVals.pack()), "9f 01 a1 61 04 c3 05 c0 05 cb 40 09 21 fb 54 41 17 44 92 01 a1 61 81 a1 6b 01 81 a1 32 a1 66 c4 01 7f d4 01 7f ca 3e 00 00 00 cf ff ff ff ff ff ff ff ff")
    }

    func testPackingNil() {
        var n: Int? = nil
        XCTAssertEqual(radix(n.pack()), "c0")
        n = 100
        XCTAssertEqual(radix(n.pack()), "64")
    }

    func testPackingStrings() {
        let a = "abcdefg"
        XCTAssertEqual(radix(a.pack()), "a7 61 62 63 64 65 66 67")
        let b = "中文"
        XCTAssertEqual(radix(b.pack()), "a6 e4 b8 ad e6 96 87")
        let c = "🤓☢☛㉿🇨🇦中Ef"
        let exp = "ba f0 9f a4 93 e2 98 a2 e2 98 9b e3 89 bf f0 9f 87 a8 f0 9f 87 a6 e4 b8 ad 45 66"
        XCTAssertEqual(radix(c.pack()), exp)
        var str8 = c
        repeat {
            str8 += c
        } while str8.utf8.count < 31 + 24
        var packed = str8.pack()
    ...
    func testPackingUInts() {
        var uint = 12
        XCTAssertEqual(radix(uint.pack()), "0c")
        uint = 127
        XCTAssertEqual(radix(uint.pack()), "7f")
        uint = 234
        XCTAssertEqual(radix(uint.pack()), "cc ea")
        uint = 65432
        XCTAssertEqual(radix(uint.pack()), "cd ff 98")
        uint = 87654321
        XCTAssertEqual(radix(uint.pack()), "ce 05 39 7f b1")
        let uint64 = UInt64.max - UInt64(0x1234)
        XCTAssertEqual(radix(uint64.pack()), "cf ff ff ff ff ff ff ed cb")
    }
